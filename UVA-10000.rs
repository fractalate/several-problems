...
fn read_problem(lines: &mut Lines<StdinLock<'static>>) -> Result<Option<Problem>, ReadProblemError> {
  ...
  let mut edges: Vec<Vec<usize>> = vec![Vec::new(); count_nodes];
  ...
  loop {
    ...
    edges[parent_node - 1].push(child_node - 1);
  }
...
fn print_solution(case_number: usize, problem: &Problem) {
  ...
  let mut stack: Vec<usize> = Vec::from([problem.start_node]);
  let mut maximum_depth = 0;
  let mut maximum_depth_node = problem.start_node;

  while let Some(parent_node) = stack.pop() {
    if nodes[parent_node].seen {
      continue;
    }

    for child_node in &problem.edges[parent_node] {
      let next_depth = nodes[*child_node].depth.max(
        nodes[parent_node].depth + 1,
      );
      nodes[*child_node].depth = next_depth;
