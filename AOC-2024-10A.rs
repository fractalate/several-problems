      ...
      if y2 < map.height && x2 < map.width {
        ...
        if map.data[y2][x2] == map.data[y][x] - 1 {
          ...
          if let Some(parent_nines) = reachable_nines.get(&(x, y)) {
            if let Some(nines) = reachable_nines.get(&(x2, y2)) {
              reachable_nines.insert((x2, y2), nines.union(&parent_nines).cloned().collect());
            } else {
              reachable_nines.insert((x2, y2), parent_nines.clone());
            }
          }
